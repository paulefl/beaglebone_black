...
var (cfgFile,host string; port int; verbose bool)
...
func Execute() {
	if err := rootCmd.Execute(); err != nil { fmt.Fprintln(os.Stderr,err); os.Exit(1) }
}
func init() {
...
func initConfig() {
	if cfgFile!=""{viper.SetConfigFile(cfgFile)}else{
		home,_:=os.UserHomeDir()
		viper.AddConfigPath(home); viper.SetConfigName(".bbcli"); viper.SetConfigType("yaml")}
	viper.AutomaticEnv(); viper.ReadInConfig()
}

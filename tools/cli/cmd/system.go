...
var systemStatusCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command,args []string) error {
		...
		if err!=nil{return fmt.Errorf("BeagleBone nicht erreichbar: %w",err)}
...
var systemRestartCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command,args []string) error {
		...
		if !force {
			...
			if a!="y"&&a!="Y"{fmt.Println("Abgebrochen."); return nil}
		}
		http.Post(apiURL()+"/api/v1/system/restart","application/json",nil)
		fmt.Print("🔄 Neustart")
		for i:=0;i<5;i++ {
			time.Sleep(time.Second); fmt.Print(".")
			if r,e:=http.Get(apiURL()+"/health");e==nil&&r.StatusCode==200{
				fmt.Println("\n✅ Wieder erreichbar!"); return nil}
		}
		fmt.Println("\n⚠️  Timeout"); return nil
	},
}

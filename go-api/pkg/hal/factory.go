...
func NewDriver(cfg *config.Config) (HardwareDriver, error) {
	...
	case "c":
		d := cdriver.New(cfg)
		if err := d.Init(); err != nil { return nil, fmt.Errorf("C Driver: %w", err) }
		log.Printf("✅ Backend: C Library")
		return d, nil
	case "rust":
		d := rustdriver.New(cfg)
		if err := d.Init(); err != nil { return nil, fmt.Errorf("Rust Driver: %w", err) }
...
func (d *autoDriver) GPIOExport(pin uint32) error {
	if err := d.primary.GPIOExport(pin); err == nil { return nil }
...
func (d *autoDriver) GPIOWrite(pin uint32, value int) error {
	if err := d.primary.GPIOWrite(pin, value); err == nil { return nil }
	return d.secondary.GPIOWrite(pin, value)
}
func (d *autoDriver) UARTOpen(port string, baud uint32) error {
	if err := d.primary.UARTOpen(port, baud); err == nil { return nil }
